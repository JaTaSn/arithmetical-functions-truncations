...
\(\omega\), is not used in this note --- the symbol \(\omega\) appears here only
once, in Remark \ref{rem:hypotheses}, and there it denotes Buchstab's function,
an unrelated object.
...
\begin{tabular}{@{}ll@{}}
  ...
  \(\omega(u)\)             & Buchstab's function; used only in Remark \ref{rem:hypotheses} \\
  \addlinespace
  ...
  \(E(n,v)\)                & composite \(p_v\)-rough integers \(\le n\), \eqref{eq:Edef} \\
...
Two collisions with standard usage are worth flagging. Here \(\omega\) is
Buchstab's function, not the number of \emph{distinct} prime factors, which this
note never uses. And \(T(n)\) is the sum of Lemma \ref{lem:split}; the
accompanying computational report \cite{gitrepo} uses the same letter for an
unrelated quantity in its \(\Om\ge3\) table.
