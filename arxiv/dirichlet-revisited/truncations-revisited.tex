...
%   2026-08-30b Claude-Code (jts-pc) -- formatting sweep, Jan's request: macros were butted
%     against symbols with no space, which typesets fine but is a pain to edit and looks
%     machine-wrought.  Three groups, verified differently because they carry different risk.
%     (a) MATH WHITESPACE, 448 spans: spaces around =, <, >, and around relation macros
%         (\le \ge \in \mid \implies \sim ...).  TeX ignores spaces in maths, so this MUST be
%         typographically neutral -- and that was checked, not assumed, by diffing pdftotext
%         output before and after: byte-identical.  '+' and '-' were deliberately left alone;
%         telling binary from unary needs real parsing, and "-1" becoming "- 1" is a regression.
%     (b) FRACTION ARGUMENTS, 39: \tfrac23 -> \tfrac{2}{3}.  The whitespace pass had first
%         turned these into "\tfrac 23", semantically identical but *worse* for the very
%         complaint that prompted the sweep -- "\tfrac 23n" reads as "23n".  Every occurrence
%         was checked to be two single tokens before bracing.  Output again byte-identical.
%     (c) TYPOGRAPHIC, 73, and these DO change line breaking: 71 non-breaking spaces tying a
%         numbered object to its reference (Theorem~\ref, Lemma~\ref, ...), one "i.e.\ ", and
%         one sentence period moved out of inline maths.  chktex flagged 196 missing ties; the
%         125 not applied are after prepositions ("of \cite{...}"), where a line break is fine
%         and a tie would be wrong.  Verified by rebuilding: same 23 pages, same 2 overfull
%         boxes as before, no new warnings.
%     chktex's other categories were checked and are false positives: all 21 "wrong length of
%     dash" are ORCID identifiers and correct en-dashed name pairs (Eliahou--Kervaire,
%     Stanley--Reisner); the bracket-mismatch warnings are half-open intervals like (n/2,n];
%     the \dots warnings ignore that amsmath already picks \cdots by context.  ONE KNOWN ITEM
%     LEFT: one sentence ends with "i.e." at a line break, where \  cannot be written --
%     harmless, fix by reflowing that sentence if it ever bothers you.
%   2026-08-30a Claude-Code (jts-pc) -- pre-submission pass.  CITATION STYLE: numeric labels
...
For a prime \(p\) and real \(y \ge 0\) put
\begin{equation}
...
\begin{theorem}\label{thm:main}
  For all \(n \ge 2\) and \(1 \le v \le \pi(n)\),
  \[
    C_{n,v} = \Phi(n,p_v),
  \]
...
\begin{proof}
  ...
  \(m = q^{a}m'\) with \(a \ge 0\) and \(m'\) free of prime factors \(\le q\);
  ...
  \begin{equation}\label{eq:Rsum}
    R_P(y) = \sum_{a \ge 0}\Phi\!\left(y/q^{a},\,q\right),
  \end{equation}
  ...
  \begin{align*}
    C_{n,v}&= R_P(n)-R_P(n/q)\\
           &= \sum_{a \ge 0}\Phi\!\left(\frac{n}{q^{a}},q\right)
            -\sum_{a \ge 0}\Phi\!\left(\frac{n}{q^{a+1}},q\right)\\
           &= \Phi(n,q).\qedhere
  \end{align*}
\end{proof}
...
\begin{example}
  ...
  \(\le 2\) are the thirteen odd numbers, so \(C_{25,1} = 13\); those with no
  prime factor \(\le 3\) are \(1,5,7,11,13,17,19,23,25\), so \(C_{25,2} = 9\);
  those with no prime factor \(\le 5\) are \(1,7,11,13,17,19,23\), so
...
\begin{proof}
  ...
  \(v \ge 1\) with \(p_v < \lpf x\), and there are \(\pi(\lpf x)-1\) of those.
\end{proof}
...
\begin{proof}
  Let \(1 \le i < j \le r\). Choose \(b \ge 1\) with \(p_i^{\,b-1} \le n < p_i^{\,b}\).
  Then
  \[
  ...
  exactly. Let \(b^{*} = \max\{\,b \ge 1 : 3^{b} \le n\,\}\), and for
  \(1 \le b \le b^{*}\) put \(a(b) = \min\{\,a \ge 1 : 2^{a}3^{b} > n\,\}\). Then
  \(x_1^{a(b)}x_2^{b}\) is a minimal generator: its weight exceeds \(n\) by
  definition; \(2^{a(b)-1}3^{b} \le n\) by minimality of \(a(b)\) when
  \(a(b) \ge 2\), and because \(3^{b} \le n\) when \(a(b) = 1\); and
  \[
    2^{a(b)}3^{\,b-1} = \tfrac{1}{3} \cdot 2^{a(b)}3^{b}
      \le \tfrac{1}{3} \cdot 2 \cdot 2^{a(b)-1}3^{b} \le \tfrac{2}{3}n \le n .
  \]
  ...
  Hence the inequality is strict as soon as \(b^{*} \ge 2\), that is, as soon as
  \(n \ge 9\); and for \(2 \le n \le 8\) equality is a finite check.
\end{proof}
...
reads ``the only integers \(x \le n\) with all prime factors \(\ge 1+r(n)-v\) are
...
\begin{proposition}\label{prop:fixed344}
  Fix \(v \ge 1\). Then \(C_{n,1+\pi(n)-v} = v\) for every \(n > 4^{\,v-1}\).
\end{proposition}
...
\begin{proof}
  ...
  gives \(p_{\pi(n)-j+1} > n/2^{\,j}\) for \(j \ge 1\), while for \(j = 0\) the same
...
\begin{proposition}\label{prop:fixed345}
  If \(n \ge 4\) is even then \(C_{n,v} = C_{n-1,v}\) for \(1 \le v \le \pi(n)\); for
...
\begin{lemma}\label{lem:secondiff}
  ...
  \(k \ge 1\) the coefficient of \(u^{k}\) is
...
\begin{lemma}\label{lem:c0}
  \(c_0 = C_{n,\pi(n)} = 1\) for every \(n \ge 2\). Consequently \(D_n(-1) = 0\).
\end{lemma}
...
\begin{proof}
  ...
  \(k \ge 1\) with \(c_k-2c_{k-1}+c_{k-2} \ne 0\). Since \(c_{-1} = 0\) and
  ...
  \(p_2^{2} = 9\), that requires \(n \ge 9\). The largest such \(v\) is then
  \[
  ...
  for every \(n \ge 4\); what needs \(n \ge 9\) is that the set of such \(v\) is
  non-empty.)

  If no such \(v\) exists --- equivalently \(\ell_1(n) = 0\), equivalently
  \(n \le 8\) --- then \(c_k = k+1\) for all
...
\begin{theorem}\label{thm:conj}
  Let \(n \ge 2\). Conjecture 4.6 of \cite{Snellman2000} holds: with
...
\begin{proof}
  ...
  The prime number theorem gives \(\pi(\sqrt n) \sim 2\sqrt n/\log n\), whence
  \[
...
\begin{proof}
  ...
  the weight \(\pi(p)-1\). If instead \(\Om(x) \ge 3\) then
  \((\lpf x)^{3} \le x \le n\), so \(\lpf x \le n^{1/3}\), and \(x\) is a multiple
  of \(\lpf x\); hence
  \[
    0 \le S(n)-S_2(n) \le \sum_{p \le n^{1/3}}\bigl(\pi(p)-1\bigr)\frac{n}{p}
    \ll n\sum_{p \le n^{1/3}}\frac 1{\log p}\ll\frac{n^{4/3}}{\log^{2}n},
  \]
...
\begin{equation}\label{eq:XM}
  X = \sqrt n,\qquad M = \log X = \tfrac{1}{2}\log n,\qquad P = \pi(\sqrt n),
\end{equation}
and we write
...
\begin{remark}\label{rem:whytwothirds}
  Lemma~\ref{lem:split} explains where the constant \(\tfrac{2}{3}\) comes from.
  Dividing \eqref{eq:split-exact} by \(P^{3}\) and expanding the second term
  \emph{exactly},
  \begin{equation}\label{eq:sqexact}
    \frac{S_2(n)}{P^{3}} = \frac{T(n)}{P^{3}}-\frac{1}{3}+\frac 1{2P}-\frac 1{6P^{2}} .
  \end{equation}
  So \(\tfrac{2}{3} = 1-\tfrac{1}{3}\), where the \(1\) is the one thing still requiring
  the prime number theorem --- that \(T(n) \sim P^{3}\), Proposition
  \ref{prop:T} below --- and the \(\tfrac{1}{3}\) is \(\sum i^{2} \sim P^{3}/3\),
  which requires nothing at all.
\end{remark}

The substitution governing \(T\) is \(p = Xe^{-a}\), under which \(n/p = Xe^{a}\);
the parameter \(a = \log(X/p) \ge 0\) measures how far below \(\sqrt n\) the
...
\begin{lemma}\label{lem:pnt-unif}
  ...
  \(|\pi(y)-y/\log y|\le A_0\,y/\log^{2}y\) \((y \ge 2)\). There is an absolute
  constant \(A_1\) such that for every \(W \ge 1\), every \(n\) with
  \(M \ge 2W\), and every real \(a \in [0,W]\),
...
\begin{proof}
  ...
  gives \(\tfrac{1}{2}M \le \log y \le \tfrac{3}{2}M\); in particular \(y \ge \sqrt X \ge M \ge 2\),
  ...
  \[
    \pi\bigl(Xe^{-a}\bigr)
      ...
      \qquad|\theta|\le 1 ,
  \]
  and since \(M-a \ge \tfrac{1}{2}M\) and \(e^{-a} \le 1\) the second term is at most
  \(4A_0X/M^{2}\). For the first, \(0 \le a/M \le \tfrac{1}{2}\) gives
  \[
    \frac{e^{-a}}{1-a/M}-e^{-a} = e^{-a} \cdot \frac{a/M}{1-a/M},
    \qquad\left|\;\cdot \;\right|\le \frac{2a}{M} \le \frac{2W}{M},
  \]
  so the first line holds with \(A_1 \ge 2+4A_0\).
...
\begin{proof}
  ...
  \emph{Range \(p \le n^{1/4}\).} Here \(\log(n/p) \ge \tfrac{3}{4}L\), so
  \[
    \pi(p)\,\pi(n/p)\ \le \ c_1^{2}\,\frac{n}{\log p \cdot \log(n/p)}
      \ \le \ \frac{4c_1^{2}}{3\log 2} \cdot \frac{n}{L} ,
  \]
  and there are at most \(\pi(n^{1/4}) \le 4c_1n^{1/4}/L\) such primes, giving a
  constant times \(n^{5/4}/L^{2}\); relative to \(X^{3}/M^{3} = 8n^{3/2}/L^{3}\)
  that is a constant times \(L/n^{1/4}\).

  \emph{Range \(n^{1/4} < p \le Xe^{-W}\).} Here \(\log p \ge \tfrac{1}{4}L\) and
  \(\log(n/p) \ge \tfrac{1}{2}L\), so \(\pi(p)\pi(n/p) \le 8c_1^{2}n/L^{2}\), while
  \[
...
\begin{theorem}\label{thm:error}
  ...
  \begin{equation}\label{eq:explicit}
    \left|\;\frac{S_2(n)}{\pi(\sqrt n)^{3}}-\frac{2}{3}\;\right|
      \ \le \ C\,\frac{\log\log n}{\sqrt{\log n}} .
  \end{equation}
  In particular, as \(n \to \infty\),
  \[
    S(n)\;\sim \;\tfrac{2}{3}\,\pi(\sqrt n)^{3}
  ...
  \[
    \frac{T(n)}{P^{3}} = \frac{M^{3}}{X^{3}}T(n) \cdot \Bigl(\frac{X}{MP}\Bigr)^{3}
      =\Bigl(1+O\bigl(\tfrac{\log M}{\sqrt M}\bigr)\Bigr)
       \Bigl(1+O\bigl(\tfrac{1}{M}\bigr)\Bigr)
      =1+O\!\left(\frac{\log M}{\sqrt M}\right)
  \]
  by Proposition~\ref{prop:T}. Substituting \(M = \tfrac{1}{2}\log n\) gives
  \eqref{eq:explicit}.

  For the asymptotic statements, \(P = \pi(\sqrt n)\) and, by the prime number
  theorem, \(\pi(\sqrt n)^{3} \sim (2\sqrt n/\log n)^{3} = 8n^{3/2}/\log^{3}n\),
  so \(S_2(n) \sim \tfrac{2}{3}\pi(\sqrt n)^{3} \sim \tfrac{16}{3}n^{3/2}/\log^{3}n\).
...
\begin{remark}\label{rem:hypotheses}
  ...
  \begin{enumerate}
    \item \emph{No sieve theory is needed, and reaching for it gives the wrong
      ...
      factor of \(\sqrt n\), that is, at \(u = \log(n/p)/\log p \to 1\); and there
      ...
      and it is uniform only for \(u \ge 2\) \cite{Fan}, which excludes precisely
      the region carrying the mass. Lemma~\ref{lem:split} says exactly what goes
      missing. On \(1 \le u \le 2\) one has \(\omega(u) = 1/u\), so the one-term
      ...
      \(\tfrac{8}{3}\,n^{3/2}/\log^{3}n\); restoring it gives
      \(8-\tfrac{8}{3} = \tfrac{16}3\). So the discrepancy is not a subtlety about
    ...
    \item \emph{The convergence is slow and not monotone}, so a numerical check
      ...
      \(S(n)\log^{3}n/n^{3/2}\) peaks near \(10.9\) around \(n = 7 \cdot 10^{4}\)
      ...
      \(\tfrac{2}{3}\pi(\sqrt n)^{3}\), which absorbs that factor entirely, the
    ...
    \item \emph{The proof uses no Riemann sums, no partition of the range, and
      ...
      \ref{lem:split}, which removes the \(\tfrac{1}{3}\) exactly, together with the
    ...
    \item \emph{The hypotheses can be weakened} if one wants only the asymptotic
      ...
      \(S(n) \sim \tfrac{2}{3}\pi(\sqrt n)^{3}\): for \(a\) in a \emph{fixed} range
      ...
      \(\varepsilon_W(n) \to 0\), uniformly in \(a\); one then lets \(W \to \infty\)
...
\begin{figure}[ht]
  ...
  \caption{The quotient \(S_2(n)\big/\tfrac{2}{3}\pi(\sqrt n)^{3}\) against \(n\),
    ...
    is a useful warning on its own. Values for \(n \le 10^{9}\) are from a
...
\begin{proposition}\label{prop:socle}
  Let \(n \ge 2\). Then \(\operatorname{soc}(\Gam_n)\) is spanned by the
  ...
  \[
    ...
    \qquad
    \dim_K\operatorname{soc}(\Gam_n) = \Bigl\lceil\frac{n}{2}\Bigr\rceil .
  \]
\end{proposition}
...
\begin{corollary}\label{cor:type}
  For \(n \ge 2\) the Cohen--Macaulay type of \(\Gam_n\) is
  \[
    \operatorname{type}(\Gam_n) = \beta^{S}_{r}(\Gam_n)
      =C_{n,1} = \Phi(n,2) = \Bigl\lceil\frac{n}{2}\Bigr\rceil,
  \]
  and \(\Gam_n\) is Gorenstein if and only if \(n \le 2\).
\end{corollary}
...
\begin{remark}
  ...
  of Betti numbers,
  \[
    D_n(t) = 1-\sum_{i \ge 1}\beta^{S}_{i}\,t^{\,i+1};
  \]
...
\begin{remark}\label{rem:unitary}
  ...
  \[
    \delta = \frac{1}{2}+\sum_{i \ge 1}
      \frac{p_i^{-1}-p_{i+1}^{-1}}{p_1p_2\cdots p_i} \approx 0.6077 .
  \]
...
\begin{remark}
  ...
  concentrated in one degree --- exactly for \(n \le 3\). Indeed for \(n \ge 4\)
  let \(2^{k}\) be the largest power of \(2\) with \(2^{k} \le n\); then
  \(2^{k} > n/2\), so \(2^{k}\) is a socle element of degree \(k \ge 2\), while
...
gave an approximation uniform for all \(x \ge y \ge 2\); see
...
letter for an unrelated quantity in its \(\Om \ge 3\) table. And two internal ones,
