...
\begin{proof}
  ...
  \]
  The largest \(v\) with this property is
  \[
  ...
  If no such \(v\) exists, i.e.\ \(\ell_1(n)=0\), then \(c_k=k+1\) for all
...
\begin{equation}\label{eq:XM}
  X=\sqrt n,\qquad M=\log X=\tfrac12\log n,
\end{equation}
and we write
...
\end{equation}
the sum being over primes. Two facts are isolated
first, so that the proof of Theorem \ref{thm:error} is a single limit argument.

The substitution that governs everything is \(p=Xe^{-a}\), under which
\(n/p=Xe^{a}\); the parameter \(a=\log(X/p)\ge0\) measures how far below
\(\sqrt n\) the smaller prime factor lies.

\begin{lemma}\label{lem:pnt-unif}
  Let \(c_1\) be an absolute constant for which \(\pi(y)\le c_1y/\log y\)
  \((y\ge2)\), as furnished by Chebyshev, and let \(A_0\) be an absolute
  constant for which \(|\pi(y)-y/\log y|\le A_0\,y/\log^{2}y\) \((y\ge2)\).
  There is an absolute constant \(A_1\) such that for every \(W\ge1\), every
  \(n\) with \(M\ge2W\), and every real \(a\in[0,W]\),
  \begin{equation}\label{eq:pnt-sub}
    \begin{aligned}
      \pi\bigl(Xe^{-a}\bigr)-1
        &=\frac XM\left(e^{-a}+\theta_1\,\frac{A_1(W+1)}{M}\right),\\
      \pi\bigl(Xe^{a}\bigr)-\pi\bigl(Xe^{-a}\bigr)+1
        &=\frac XM\left(2\sinh a+\theta_2\,\frac{A_1(W+1)e^{W}}{M}\right),
    \end{aligned}
  \end{equation}
  where \(|\theta_1|,|\theta_2|\le1\).

  The point of the second bound is that its error constant grows with \(W\),
  because \(2\sinh a\) itself does; this is what will limit how fast \(W\) may
  be allowed to grow with \(n\) in Theorem \ref{thm:error}.
\end{lemma}

\begin{proof}
  For \(y=Xe^{\mp a}\) we have \(\log y=M\mp a\), and \(0\le a\le W\le M/2\)
  gives \(\tfrac12M\le\log y\le\tfrac32M\).
  ...
  \[
    \pi\bigl(Xe^{-a}\bigr)
      ...
      \qquad|\theta|\le1 .
  \]
  Since \(M-a\ge\tfrac12M\) and \(e^{-a}\le1\), the second term is at most
  \(4A_0X/M^{2}\). For the first, \(0\le a/M\le\tfrac12\) gives
  \[
    \frac{e^{-a}}{1-a/M}-e^{-a}
      =e^{-a}\cdot\frac{a/M}{1-a/M},
    \qquad\left|\;\cdot\;\right|\le\frac{2a}{M}\le\frac{2W}{M},
  \]
  so
  \[
    \pi\bigl(Xe^{-a}\bigr)
      =\frac XM\left(e^{-a}+\theta_1'\,\frac{2W+4A_0}{M}\right),
      \qquad|\theta_1'|\le1 .
  \]
  It remains to subtract \(1\). That changes the left side by \(1\), which is
  absorbed by the error term as soon as \((X/M)(1/M)\ge1\), i.e.\ \(X\ge M^{2}\),
  and this holds for all large \(n\) since \(M=\tfrac12\log n\). So the first
  line of \eqref{eq:pnt-sub} holds with \(A_1=2+4A_0+1\).

  \emph{Second line.} The same computation with \(\log y=M+a\), using
  \(M+a\le\tfrac32M\) and \(e^{a}\le e^{W}\), gives
  \[
    \pi\bigl(Xe^{a}\bigr)
      =\frac XM\left(e^{a}+\theta'\,\frac{(W+A_0)e^{W}}{M}\right),
      \qquad|\theta'|\le1 .
  \]
  Subtracting the first line and using \(e^{a}-e^{-a}=2\sinh a\) yields the
  second, the two error terms combining into one of size at most
  \(A_1(W+1)e^{W}/M\).
\end{proof}
...
\begin{lemma}\label{lem:tail}
  ...
  \(1\le W\le M/2\),
  \[
    \sum_{p\le Xe^{-W}}g(p)\ \le\ c_0\,e^{-W}\,\frac{n^{3/2}}{\log^{3}n}
      \ +\ o\!\left(\frac{n^{3/2}}{\log^{3}n}\right),
  \]
  the \(o\)-term being independent of \(W\).
\end{lemma}

\begin{proof}
  Split at \(n^{1/4}\), and write \(L=\log n=2M\) throughout.

  \emph{Range \(p\le n^{1/4}\).} Here \(\log(n/p)\ge\tfrac34L\), so by
  Chebyshev
  \[
    g(p)\ \le\ \pi(p)\,\pi(n/p)
      \ \le\ c_1^{2}\,\frac{n}{\log p\cdot\log(n/p)}
      \ \le\ \frac{4c_1^{2}}{3\log2}\cdot\frac nL .
  \]
  There are at most \(\pi(n^{1/4})\le 4c_1n^{1/4}/L\) such primes, so this
  range contributes at most a constant times \(n^{5/4}/L^{2}\). Since
  \[
    \frac{n^{5/4}/L^{2}}{n^{3/2}/L^{3}}=\frac{L}{n^{1/4}}\longrightarrow0,
  \]
  that is \(o(n^{3/2}/\log^{3}n)\), uniformly in \(W\).

  \emph{Range \(n^{1/4}<p\le Xe^{-W}\).} Here \(\log p\ge\tfrac14L\) and
  \(\log(n/p)\ge\tfrac12L\), so
  \[
    g(p)\ \le\ \pi(p)\,\pi(n/p)\ \le\ c_1^{2}\,\frac{n}{\log p\cdot\log(n/p)}
      \ \le\ 8c_1^{2}\,\frac{n}{L^{2}} .
  \]
  The number of such primes is at most
  \[
    \pi\bigl(Xe^{-W}\bigr)\ \le\ c_1\,\frac{Xe^{-W}}{M-W}
      \ \le\ 2c_1\,\frac{Xe^{-W}}{M}\ =\ 4c_1\,\frac{Xe^{-W}}{L},
  \]
  using \(W\le M/2\). Multiplying the two bounds gives at most
  \[
    32c_1^{3}\,e^{-W}\,\frac{nX}{L^{3}}
      =32c_1^{3}\,e^{-W}\,\frac{n^{3/2}}{L^{3}},
  \]
  so \(c_0=32c_1^{3}\) will do.
\end{proof}

\begin{theorem}\label{thm:error}
  There are absolute constants \(C\) and \(n_0\) such that for all \(n\ge n_0\)
  \begin{equation}\label{eq:explicit}
    \left|\;\frac{M^{3}}{X^{3}}\,S_2(n)-\frac23\;\right|
      \ \le\ C\,\frac{\log\log n}{\sqrt{\log n}} ,
  \end{equation}
  with \(X,M\) as in \eqref{eq:XM}. In particular, as \(n\to\infty\),
  \[
...
\begin{proof}
  By Lemma \ref{lem:semiprime} it suffices to treat \(S_2\), because
  \[
    \frac{n^{4/3}/\log^{2}n}{n^{3/2}/\log^{3}n}
      =\frac{\log n}{n^{1/6}}\longrightarrow0 .
  \]

  \emph{Why the mass is where it is.} Under \(p=Xe^{-a}\) the two factors of
  \(g\) are given by \eqref{eq:pnt-sub}, and the number of primes per unit
  \(a\) near \(p\) is about \(Xe^{-a}/M\). The product of the three is
  \[
    \frac{X^{3}}{M^{3}}\cdot 2e^{-2a}\sinh a ,
  \]
  which decays like \(e^{-a}\) as \(a\to\infty\). So the sum is concentrated on
  \(a=O(1)\): both \(p\) and \(n/p\) lie within a bounded factor of \(\sqrt n\).
  Lemma \ref{lem:tail} is the rigorous form of that statement, and the rest of
  the proof evaluates the main range.

  \emph{The main range.} Fix \(W\ge1\) and \(K\ge1\), and assume \(n\) large
  enough that \(M\ge2W\). Cut \([0,W]\) into \(K\) equal pieces
  \(a_i=iW/K\) \((0\le i\le K)\) and put \(t_i=Xe^{-a_i}\), so that
  \(X=t_0>t_1>\dots>t_K=Xe^{-W}\). On \([t_{i+1},t_i]\) the factor
  \(\pi(t)-1\) is non-decreasing in \(t\) and \(\pi(n/t)-\pi(t)+1\) is
  non-increasing, so
  \begin{align*}
    m_i&:=\bigl(\pi(t_{i+1})-1\bigr)\bigl(\pi(n/t_i)-\pi(t_i)+1\bigr),\\
    M_i&:=\bigl(\pi(t_i)-1\bigr)\bigl(\pi(n/t_{i+1})-\pi(t_{i+1})+1\bigr)
  \end{align*}
  satisfy \(m_i\le g(p)\le M_i\) for every prime \(p\in(t_{i+1},t_i]\). Writing
  \(N_i=\pi(t_i)-\pi(t_{i+1})\) for the number of such primes,
  \begin{equation}\label{eq:sandwich}
    \sum_{i=0}^{K-1}m_iN_i\ \le\ \sum_{Xe^{-W}<p\le X}g(p)\ \le\
    \sum_{i=0}^{K-1}M_iN_i .
  \end{equation}
  Every prime count appearing in \eqref{eq:sandwich} is evaluated at an
  argument of the form \(Xe^{\pm a}\) with \(0\le a\le W\), so Lemma
  \ref{lem:pnt-unif} applies to all of them with the \emph{same} constant \(c(W)\);
  there are \(O(K)\) of them. Substituting \eqref{eq:pnt-sub} and expanding,
  both outer members of \eqref{eq:sandwich} equal
  \begin{equation}\label{eq:riemann}
    \frac{X^{3}}{M^{3}}\sum_{i=0}^{K-1}
      2e^{-a_i}\sinh(a_i)\bigl(e^{-a_i}-e^{-a_{i+1}}\bigr)
    \;+\;R_1\;+\;R_2,
  \end{equation}
  where the two error terms are controlled separately:
  \begin{itemize}
    \item \(|R_1|\le c'(W,K)\,X^{3}/M^{4}\), collecting every cross term in
      which at least one factor is an error term of \eqref{eq:pnt-sub}. There
      are \(O(K)\) such terms, each of size \(O_W(X^{3}/M^{4})\), and \(K\) is
      fixed before \(n\) grows.
    \item \(|R_2|\le c''(W)\,\dfrac{X^{3}}{M^{3}}\cdot\dfrac WK\), from
      replacing \(g\) by its one-sided extreme on each piece: the integrand
      \(2e^{-2a}\sinh a\) has bounded derivative on \([0,W]\), so \(M_i-m_i\)
      contributes \(O_W(W/K)\) in total.
  \end{itemize}
  The sum in \eqref{eq:riemann} is a Riemann--Stieltjes sum for
  \(\int_0^W2e^{-2a}\sinh a\,\mathrm da\) with respect to
  \(\mathrm d(-e^{-a})\), over a partition of mesh \(W/K\).

  \emph{Collecting, with \(W\) and \(K\) chosen.} Write
  \(F(a)=2e^{-2a}\sinh a=e^{-a}-e^{-3a}\), so that \(|F'|\le3\) everywhere and
  \(\int_0^{\infty}F=1-\tfrac13=\tfrac23\). Four quantities separate
  \(M^{3}S_2(n)/X^{3}\) from \(\tfrac23\), and each is now bounded explicitly,
  with \(C_1,\dots,C_4\) absolute:
  \begin{enumerate}
    \item \emph{the tail}, \(\sum_{p\le Xe^{-W}}g(p)\): at most
      \(C_1e^{-W}\) by Lemma \ref{lem:tail}, together with the term
      \(C_1\log n\,/\,n^{1/4}\) from its range \(p\le n^{1/4}\);
    \item \emph{the truncation of the integral},
      \(\int_W^{\infty}F\le\int_W^{\infty}e^{-a}\,\mathrm da=e^{-W}\);
    \item \emph{the mesh}, \(R_2\): at most \(C_2W/K\), since \(|F'|\le3\) and
      the total variation of \(-e^{-a}\) is at most \(1\);
    \item \emph{the prime number theorem}, \(R_1\): at most
      \(C_3(W+1)e^{W}/M\), from \eqref{eq:pnt-sub} --- each of the
      \(\pi(X)\le C_4X/M\) primes in the main range contributes an error
      \((X^{2}/M^{2})\cdot A_1(W+1)e^{W}/M\), because \(e^{-a}\le1\) and
      \(2\sinh a\le e^{W}\).
  \end{enumerate}
  Now make the choice
  \begin{equation}\label{eq:WK}
    W=\tfrac12\log M,\qquad K=\lceil M\rceil ,
  \end{equation}
  legitimate for \(M\ge e^{2}\), since then \(W\ge1\) and \(M\ge2W\). With it,
  \[
    e^{-W}=\frac1{\sqrt M},\qquad
    \frac WK\le\frac{\log M}{2M},\qquad
    \frac{(W+1)e^{W}}{M}=\frac{\tfrac12\log M+1}{\sqrt M},
  \]
  all three of which are \(O(\log M/\sqrt M)\); and \(\log n/n^{1/4}\) is
  smaller than any of them. Hence
  \[
    \left|\frac{M^{3}}{X^{3}}S_2(n)-\frac23\right|
      \ \le\ C\,\frac{\log M}{\sqrt M} ,
  \]
  which is \eqref{eq:explicit} after substituting \(M=\tfrac12\log n\).

  \emph{Conversion.} By \eqref{eq:XM},
  \[
    \frac{X^{3}}{M^{3}}=\frac{n^{3/2}}{(\tfrac12\log n)^{3}}
      =8\,\frac{n^{3/2}}{\log^{3}n},
  \]
  and \(X/M\sim\pi(\sqrt n)\), so \eqref{eq:explicit} gives
  \(S_2(n)\sim\tfrac23\pi(\sqrt n)^{3}\sim\tfrac{16}{3}n^{3/2}/\log^{3}n\).
  Lemma \ref{lem:semiprime} transfers this to \(S\), and Corollary
  \ref{cor:total} to \(C_n\).
\end{proof}

\begin{remark}\label{rem:hypotheses}
  Two comments on what the proof of Theorem \ref{thm:error} actually uses.

  The rate in \eqref{eq:explicit} is certainly not sharp: it is limited by the
  crude truncation at \(a=W\), not by the arithmetic input. Sharpening it would
  mean treating the whole range \(0\le a\le M\) rather than discarding
  \(a>W\), which is a different and more delicate calculation. In fact the true
  deviation is of order \(1/\log n\), far smaller than
  \(\log\log n/\sqrt{\log n}\): at \(n=10^{14}\) the left-hand side of
  \eqref{eq:explicit} is \(0.167\) against a bound of \(0.692\).

  A reader comparing \eqref{eq:explicit} with Figure \ref{fig:ratio} should
  note that the two normalise differently, and that the difference is not
  small at any computable \(n\). Figure \ref{fig:ratio} divides by
  \(\tfrac23\pi(\sqrt n)^{3}\), whereas \eqref{eq:explicit} divides by
  \(X^{3}/M^{3}=(\sqrt n/\log\sqrt n)^{3}\); and
  \(\pi(\sqrt n)\big/(X/M)=1+1/M+O(1/M^{2})\), so the two differ by a factor
  \(1+3/M+O(1/M^{2})\). At \(n=10^{6}\) that is already a \(16\%\) discrepancy
  in \(\pi(\sqrt n)\) and \(56\%\) once cubed. This is the same effect as the
  one described in Remark \ref{rem:buchstab-trap}, and it is why
  \eqref{eq:explicit} still reads \(1.10\) rather than \(0.67\) at
  \(n=10^{6}\) while Figure \ref{fig:ratio} already reads \(1.06\).

  Conversely, the hypotheses can be weakened if one wants only the asymptotic
  and no rate. Lemma \ref{lem:pnt-unif} was stated with the prime number
  theorem in the sharp form \(\pi(y)=y/\log y+O(y/\log^{2}y)\), because that is
  what produces an explicit error. But the bare prime number theorem,
  \(\pi(y)\sim y/\log y\), already suffices for
  \(S(n)\sim\tfrac23\pi(\sqrt n)^{3}\): for \(a\) in a \emph{fixed} range
  \([0,W]\) the arguments \(Xe^{\pm a}\) all tend to infinity with \(n\), so
  \(\pi(Xe^{\mp a})(M\pm a)/(Xe^{\mp a})\to1\) uniformly in \(a\), which turns
  \eqref{eq:pnt-sub} into the same statement with \(A_1(W+1)e^{W}/M\) replaced
  by an unspecified \(\varepsilon_W(n)\to0\); every later step uses only that
  the relative errors vanish. This matters for anyone wishing to formalise the
  argument in a proof assistant, since it removes the need for a prime number
  theorem \emph{with} an error term.
\end{remark}

\begin{remark}\label{rem:buchstab-trap}
  Two things about this deserve to be said, because both are traps.

  First, no sieve theory is needed, and reaching for it gives the wrong answer.
  The mass sits where \(p\) and \(n/p\) are both within a bounded factor of
  \(\sqrt n\), that is, at \(u=\log(n/p)/\log p\to1\); and there \(\Phi\)
  degenerates to the exact identity \(\Phi(x,y)=\pi(x)-\pi(y)+1\), valid for
  \(y\ge\sqrt x\). The one-term Buchstab estimate
  \(\Phi(x,y)\approx x\,\omega(u)/\log y\), fed into \eqref{eq:Sdef}, returns
  \(8\) rather than \(16/3\): it is uniform only for \(u\ge2\)
  \cite{Fan}, which excludes precisely the region carrying the mass, and there
  the secondary term \(-y/\log y\) is of the same order and restores
  \(8-8/3=16/3\).

  Second, the convergence is slow and not monotone, so a numerical check over
  too short a range is misleading. The raw quotient
  \(S(n)\log^{3}n/n^{3/2}\) peaks near \(10.9\) around \(n=10^{5}\) and is
  still \(9.55\) at \(n=10^{7}\), against a limit of \(16/3=5.33\dots\); the
  discrepancy is almost all in the difference between \(\pi(\sqrt n)\) and
  \(2\sqrt n/\log n\), which is cubed here. Measured instead against
  \(\tfrac23\pi(\sqrt n)^{3}\) the picture is much better behaved, but still
  not monotone --- see Figure \ref{fig:ratio}. A second-order factor
  \(1+6/\log n\) accounts for the shape.
\end{remark}
...
\begin{proof}
  ...
  for all \(i\), that system of conditions is equivalent to its weakest member,
  the one at \(i=1\): \(2w(u)>n\). So the socle monomials are those of weight
...
\begin{remark}\label{rem:unitary}
  ...
  \cite[\S7]{SnellmanUnitaryTrunc} obtains the socle dimension only
  asymptotically, as \(\delta n\) with
  \[
    \delta=\frac12+\sum_{i\ge1}
      \frac{p_i^{-1}-p_{i+1}^{-1}}{p_1p_2\cdots p_i}\approx0.6077 .
  \]
  For Dirichlet convolution the corresponding density is exactly \(1/2\), for
  every \(n\). The asymmetry between the two convolutions is the interesting
...
Macaulay's theorem the sequence \(d\mapsto\#\{m\le n : \Om(m)=d\}\) --- the
Landau counts --- must be an \(M\)-sequence, which yields growth bounds free of
...
cautions of Remark \ref{rem:buchstab-trap}; and
