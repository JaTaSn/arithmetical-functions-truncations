...
\begin{proof}
  ...
  equals \(\#\{p\text{ odd} : p^{2}\le n\}=\ell_1(n)\), the two counts
...
\begin{proof}
  Write \(X=\sqrt n\) and \(M=\log X=\tfrac12\log n\). By Lemma
  \ref{lem:semiprime} it suffices to treat \(S_2\), since
  \(n^{4/3}\log^{3}n/(n^{3/2}\log^{2}n)=n^{-1/6}\log n\to0\).

  Substitute \(p=Xe^{-a}\), so \(n/p=Xe^{a}\). The prime number theorem in the
  form \(\pi(x)=x/\log x+O(x/\log^{2}x)\) gives, uniformly for
  \(0\le a\le W\) with \(W\) fixed,
  \begin{equation}\label{eq:pnt-sub}
    \begin{aligned}
      \pi(p)-1&=\frac XM\bigl(e^{-a}+O_W(M^{-1})\bigr),\\
      \pi(n/p)-\pi(p)+1&=\frac XM\bigl(2\sinh a+O_W(M^{-1})\bigr),
    \end{aligned}
  \end{equation}
  and the number of primes near \(p\) per unit \(a\) is \(\approx Xe^{-a}/M\).
  The summand times that density is \(\approx (X^{3}/M^{3})\,2e^{-2a}\sinh a\),
  which decays like \(e^{-a}\): the sum is concentrated on \(a=O(1)\), that is,
  on \(p\) and \(n/p\) both within a bounded factor of \(\sqrt n\).

  Make this precise by splitting \(\sum_{p\le X}g(p)\), where
  \(g(t)=(\pi(t)-1)(\pi(n/t)-\pi(t)+1)\), into three ranges. For
  \(p\le n^{1/4}\) we have \(g(p)\ll n/\log n\) and there are
  \(\ll n^{1/4}/\log n\) such primes, contributing
  \(\ll n^{5/4}/\log^{2}n=o(n^{3/2}/\log^{3}n)\). For
  \(n^{1/4}<p\le Xe^{-W}\) we have \(g(p)\le\pi(p)\pi(n/p)\ll n/\log^{2}n\),
  and Chebyshev's bound gives \(\pi(Xe^{-W})\ll Xe^{-W}/\log n\), so this range
  contributes at most \(c_0e^{-W}n^{3/2}/\log^{3}n\) with \(c_0\) absolute.

  In the main range \(Xe^{-W}<p\le X\), cut \([0,W]\) into \(K\) equal pieces
  \(a_i=iW/K\) and put \(t_i=Xe^{-a_i}\). The factor \(\pi(t)-1\) is
  non-decreasing and \(\pi(n/t)-\pi(t)+1\) non-increasing in \(t\), so on each
  piece \(g\) is trapped between the products of the one-sided extremes, and
  \(\sum_{Xe^{-W}<p\le X}g(p)\) lies between
  \(\sum_i\min g\cdot(\pi(t_i)-\pi(t_{i+1}))\) and the corresponding sum with
  \(\max\). Every prime count occurring is evaluated at an argument
  \(Xe^{\pm O(W)}\), so \eqref{eq:pnt-sub} applies to all of them with one
  implied constant, and both bounds become
  \[
    \frac{X^{3}}{M^{3}}\sum_{i=0}^{K-1}
      2e^{-a_i}\sinh(a_i)\bigl(e^{-a_i}-e^{-a_{i+1}}\bigr)
    +O_{W,K}\!\left(\frac{X^{3}}{M^{4}}\right)
    +O_W\!\left(\frac{X^{3}}{M^{3}}\cdot\frac WK\right),
  \]
  the displayed sum being a Riemann--Stieltjes sum for
  \(\int_0^W 2e^{-2a}\sinh a\,\mathrm da\). Letting \(n\to\infty\), then
  \(K\to\infty\), then \(W\to\infty\), the three ranges combine to give
  \[
    \frac{M^{3}}{X^{3}}S_2(n)\longrightarrow
      ...
      =1-\tfrac13=\tfrac23 .
  \]
  Finally \(X/M\sim\pi(\sqrt n)\) and \(X^{3}/M^{3}=8n^{3/2}/\log^{3}n\).
\end{proof}
...
\begin{remark}\label{rem:buchstab-trap}
  ...
  \(\tfrac23\pi(\sqrt n)^{3}\), the quotient \(S_2(n)/\tfrac23\pi(\sqrt n)^3\)
  is \(1.0593\) at \(n=10^{6}\) and \(1.0853\) at \(10^{7}\), rises to a
  maximum, and then falls through \(1.0492,1.0353,1.0313,1.0255,1.0204,1.0171\)
  at \(n=10^{9},\dots,10^{14}\). A second-order factor \(1+6/\log n\) accounts
  for the shape.
\end{remark}
...
\begin{remark}
  ...
  of Betti numbers, \(D_n(t)=1-\sum_{i\ge1}\beta^{S}_{i}t^{i+1}\); since
  \(\operatorname{pd}_S\Gam_n=r\), its leading coefficient is
