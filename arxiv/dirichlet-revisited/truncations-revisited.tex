...
\begin{lemma}\label{lem:pnt-unif}
  ...
  \(M\ge2W\) and \(X\ge M^{2}\), and every real \(a\in[0,W]\),
...
\begin{proof}
  ...
  \(M+a\le\tfrac32M\) and \(e^{a}\le e^{W}\), gives an error at most
  \((W+A_0)e^{W}/M\) after division by \(X/M\).
\end{proof}
...
\begin{proof}
  ...
  smaller than the claimed bound. By Lemma \ref{lem:pnt-unif} with \(a=0\),
  \(P=\pi(X)=(X/M)(1+\theta A_1/M)\), so
  \[
...
\begin{remark}\label{rem:hypotheses}
  ...
  arithmetic input, and the true deviation is of order \(1/\log n\): at
  \(n=10^{14}\) the left-hand side of \eqref{eq:explicit} is \(0.011\) against
  a bound of \(0.69\). Sharpening it would mean treating the whole range
