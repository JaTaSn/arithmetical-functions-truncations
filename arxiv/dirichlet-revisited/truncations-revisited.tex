...
\begin{equation}\label{eq:gdef}
  ...
  \qquad\text{so that}\qquad S_2(n)=\sum_{p\le X}g(p)
\end{equation}
by Lemma \ref{lem:semiprime}, the sum being over primes. Two facts are isolated
...
\begin{lemma}\label{lem:pnt-unif}
  ...
  \((y\ge2)\), as furnished by Chebyshev. Fix \(W\ge1\). There is a constant
  \(c(W)\), depending on \(W\) alone, such that for every \(n\) with
  \(M\ge2W\) and every real \(a\in[0,W]\),
  \begin{equation}\label{eq:pnt-sub}
    \begin{aligned}
      \pi\bigl(Xe^{-a}\bigr)-1
        &=\frac XM\left(e^{-a}+\theta_1\frac{c(W)}{M}\right),\\
      \pi\bigl(Xe^{a}\bigr)-\pi\bigl(Xe^{-a}\bigr)+1
        &=\frac XM\left(2\sinh a+\theta_2\frac{c(W)}{M}\right),
    \end{aligned}
  \end{equation}
...
\begin{proof}
  Write the prime number theorem as \(\pi(y)=y/\log y+O(y/\log^{2}y)\). For
  \(y=Xe^{\mp a}\) we have \(\log y=M\mp a\), and \(0\le a\le W\le M/2\) gives
  \(M/2\le\log y\le\tfrac32M\); in particular \(\log y\asymp M\) with absolute
  constants. Hence
  \[
    \pi\bigl(Xe^{-a}\bigr)
      =\frac{Xe^{-a}}{M-a}+O\!\left(\frac{X}{M^{2}}\right)
      =\frac XM\cdot\frac{e^{-a}}{1-a/M}+O\!\left(\frac{X}{M^{2}}\right).
  \]
  Since \(0\le a/M\le\tfrac12\), the expansion \(1/(1-a/M)=1+O(a/M)\) holds with
  an absolute constant, and \(a\le W\), so
  \[
    \frac{e^{-a}}{1-a/M}=e^{-a}+O\!\left(\frac WM\right).
  \]
  Subtracting \(1\) changes nothing at this precision, and the first line of
  \eqref{eq:pnt-sub} follows on choosing \(c(W)\) larger than all the absolute
  constants involved, times \(W\). The same computation with \(\log y=M+a\)
  gives \(\pi(Xe^{a})=(X/M)(e^{a}+O(W/M))\), and subtracting the first line
  yields the second, since \(e^{a}-e^{-a}=2\sinh a\).
\end{proof}
...
\begin{proof}
  ...
  \emph{The three limits.} Let \(n\to\infty\) with \(W,K\) fixed: \(R_1/(X^3/M^3)\to0\).
  Then let \(K\to\infty\): \(R_2/(X^3/M^3)\to0\) and the Riemann sum tends to
  the integral. Finally let \(W\to\infty\): by Lemma \ref{lem:tail} the omitted
  range contributes at most \(c_0e^{-W}\), which tends to \(0\), and the
  integral converges. Combining,
  \[
    \frac{M^{3}}{X^{3}}\,S_2(n)\ \longrightarrow\
      \int_0^{\infty}2e^{-2a}\sinh a\,\mathrm da
      =\int_0^{\infty}\bigl(e^{-a}-e^{-3a}\bigr)\,\mathrm da
      =1-\tfrac13=\tfrac23 .
  \]

  \emph{Conversion.} By the prime number theorem \(X/M\sim\pi(\sqrt n)\), and
  by \eqref{eq:XM}
  \[
